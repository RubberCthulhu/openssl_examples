...
int main(int argc, char **argv)
{
	...
	fprintf(stderr, "Error: EC_KEY_new_by_curve_name(%d): %s\n", nid, ERR_error_string(ERR_get_error(), NULL));
	exit(EXIT_FAILURE);
    }
    
    if( EC_KEY_generate_key(eckey) != 1 ) {
	fprintf(stderr, "Error: EC_KEY_generate_key(<out-key>): %s\n", ERR_error_string(ERR_get_error(), NULL));
	exit(EXIT_FAILURE);
    }
    
    key = EVP_PKEY_new();
    if( EVP_PKEY_set1_EC_KEY(key, eckey) != 1 ) {
	fprintf(stderr, "Error: EVP_PKEY_set1_EC_KEY(<out-key>): %s\n", ERR_error_string(ERR_get_error(), NULL));
	exit(EXIT_FAILURE);
    }
